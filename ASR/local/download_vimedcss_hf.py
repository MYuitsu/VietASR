...
DEFAULT_DATASET = "tensorxt/ViMedCSS"
DEFAULT_SPLITS = ("train", "validation", "test", "hard")
DEFAULT_LOCAL_CACHE_ROOT = Path(
    "/home/nguyenthaiduy277/.cache/huggingface/datasets/tensorxt___vi_med_css"
)
...
def find_local_arrow(local_cache_root: Path, split_name: str) -> Path:
    matches = sorted(local_cache_root.glob(f"**/*-{split_name}.arrow"))
    if not matches:
...
def load_split_dataset(
    ...
    try:
        logging.info(
            "Loading %s split=%s from Hugging Face datasets", dataset_name, split_name
        )
        ds = load_dataset(
            dataset_name,
            split=split_name,
        ...
        logging.warning(
            "Falling back to local cached arrow for split=%s due to: %s",
            split_name,
            exc,
        )
...
def write_split(
    ...
    for index, item in enumerate(ds):
        audio = item["audio"]
        text = item.get("segment_text") or item.get("text")
        if not text:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download the labeled ViMedCSS splits from Hugging Face and convert them to VietASR supervised format."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("ASR/download_vimedcss"),
        help="Target ASR download root for ViMedCSS in VietASR supervised format.",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
    ...
    logging.info("Finished downloading ViMedCSS. Total utterances: %s", total)
